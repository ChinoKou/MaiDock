...
MaiDock 是 MaiBot 的独立 LLM Provider 插件，为 OpenAI、Anthropic、Volcengine ARK、DashScope、SiliconFlow 和 Xiaomi Mimo 提供原生 `httpx` 后端，运行时不依赖供应商 SDK。本指南作用于整个 MaiDock 仓库。
...
- `src/plugin.py`：`MaiDockPlugin`、六个 `@LLMProvider` 注册入口和插件生命周期。
...
- `src/providers/common/`：HTTP、参数管线、音视频校验和协议无关解析原语。
- `src/providers/*_family/`：协议级请求、响应、流式、工具、多模态、ASR/Embedding 复用层。
- `src/providers/*_provider/`：端点、鉴权、模型限制、默认值和供应商特殊行为。
- `docs/`、`README.md`：配置、extra params、模型示例、能力矩阵、架构说明和开发文档；`docs/provider_docs/` 是按需生成且被 Git 忽略的本地供应商资料。
- `scripts/`：配置生成和供应商文档维护工具；`tests/` 是纳入版本管理的正式测试套件。

入口链路为：

```text
plugin.py -> src/plugin.py -> MaiDockPlugin -> Provider.dispatch()
```

Provider 首次使用时惰性创建，配置热更新只使 Provider 实例失效。插件级 `PluginStateStore` 在 `on_load` 获得标准数据目录，固定使用 `ctx.paths.data_dir / "maidock_state.sqlite3"`，首次读写时打开，并仅在 `on_unload` 关闭。
...
```text
具体 Provider -> 对应协议 Family -> common -> Core / schemas
```
...
```text
<name>_provider/
  provider.py              # LLMProviderBase、dispatch 和能力入口
...
- `common/`：HTTP、参数管线、音频签名、图片处理、向量校验和协议无关解析。

### Core 与 Schema 职责

| 模块 | 用途 |
| --- | --- |
| `core/json_types.py` | `JsonValue`、JSON 窄化、取值和规范化 |
| `core/parameter_catalog.py` | extra params 字段、目标路径、类型和 source aliases |
| `core/parameter_policy.py` | Provider/能力粒度的接受、拒绝、禁用和覆写规则 |
| `core/common.py` | `ProviderRuntimeOptions`、URL、图片、usage、客户端通用配置 |
...
1. Host 提供请求快照和 `model_info.extra_params`。
2. `build_normalized_host_parameters()` 按 policy defaults、model extra、model typed、request extra、request typed 的优先级合并。
3. Provider 翻译器生成 `TranslationEnvelope` 的 body、headers 和 query。
4. `apply_transport_parameter_policy()` 应用 disabled、rejected 和 override 规则。
5. 未知字段按 `unknown_extra_params = forward/drop/reject` 处理。

每个 `[{provider}.{capability}]` 策略包含：

| 字段 | 含义 |
| --- | --- |
| `accept_model_extra_params` | 是否接收模型级 extra params |
| `accept_request_extra_params` | 是否接收请求级 extra params |
| `unknown_extra_params` | 未知字段转发、丢弃或拒绝 |
| `disabled_paths` | 静默移除的路径 |
| `rejected_paths` | 出现即报错的路径 |

`fields` 子段提供逐字段 enabled/override 控制；`default_params` 是最低优先级默认值；`override_params` 是最高优先级强制值，可分别作用于 body、headers 和 query。新增字段必须同时更新参数目录、Provider 翻译器、配置 Schema、模板、参考文档与请求测试。
