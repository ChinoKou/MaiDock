...
from ...core.json_types import JsonValue, json_array, json_mapping_or_none, list_field, mapping_field
...
class ChatCompletionsMapper:
    ...
    def build_request_body(
        ...
        *,
        ...
        overrides = self.options.parameter_overrides.get(self.policy_provider, "chat_completion")
        ...
        body: dict[str, JsonValue] = {"model": model, "messages": json_array(messages), "stream": stream}
        if tools:
            body["tools"] = json_array(tools)
    ...
    def _convert_messages(self, request: ResponseRequestSnapshot) -> list[dict[str, JsonValue]]:
        converted: list[dict[str, JsonValue]] = []
        for message in request.message_list:
            if message.role not in {"system", "user", "assistant", "tool"}:
                continue
            if message.role == "tool":
                tool_message: dict[str, JsonValue] = {"role": "tool", "content": message_text(message)}
            ...
            current: dict[str, JsonValue] = {
                "role": message.role,
                "content": content if content is None or isinstance(content, str) else json_array(content),
            }
    ...
    def convert_response(self, payload: dict[str, JsonValue]) -> ProviderResponse:
    ...
    @staticmethod
    def _extract_usage(payload: dict[str, JsonValue]) -> ProviderUsage:
