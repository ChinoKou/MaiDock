...
from ...core.json_types import json_list_or_none, normalize_json_value
...
def translate_chat_response_format(
    ...
    if response_format is None:
        return
    # 覆写值在 normalized.fields 中排在 Host 类型字段之后，这里直接覆盖，
    # 实现「MaiDock 覆写值 > Core 类型字段」的叶级合并。
...
def translate_chat_tools(
    ...
    if incoming_tools is None:
        # 单个工具也接受：非列表值包成单元素列表，顺手窄化成 JSON 值（对 JSON 输入恒等）。
