...
from ...clients.ark import ArkSession
...
class PrefixCacheManager:
    ...
    async def _create_entry(
        ...
        *,
        ...
        create_body: dict[str, JsonValue] = {
            "model": plan.model,
            "input": json_array(plan.prefix_input),
            "store": True,
            "caching": {"type": "enabled", "prefix": True},
            "expire_at": expire_at,
        }
        if plan.tools:
            create_body["tools"] = json_array(plan.tools)
        ...
        await self._state_store.set(
            ...
            plan.cache_key,
            value_to_json_object(entry.model_dump(mode="json")),
    ...
    async def _count_prefix_tokens(
        ...
        *,
        ...
        # 单条文本直接传字符串、多条传数组是 ARK tokenization 的入参约定。
        # 这里标成 JsonValue 而不是 str | list[str]：list 在类型系统里是不变的，
        # list[str] 并不是 list[JsonValue]，直接塞进请求体会被判不兼容。
        text: JsonValue = (
            plan.tokenization_texts[0] if len(plan.tokenization_texts) == 1 else list(plan.tokenization_texts)
        )
        payload = await session.tokenization.create(
            json_resource_request(
                {"model": plan.model, "text": text},
                headers=self._fresh_request_headers(headers),
                query=query,
            ),
            retry=session.retry,
