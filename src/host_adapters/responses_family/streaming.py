...
class ResponsesStreamAccumulator:
    ...
    def merge_tool_item(self, item: dict[str, JsonValue]) -> None:
    ...
    def append_tool_arguments(self, event: dict[str, JsonValue], delta: str) -> None:
        if not delta:
            return
        tool = self._tool_for_event(event)
        tool.append_arguments(delta)

    def set_tool_arguments(self, event: dict[str, JsonValue], arguments: str) -> None:
        tool = self._tool_for_event(event)
        tool.set_arguments(arguments)
    ...
    def _tool_for_event(self, event: dict[str, JsonValue]) -> ResponsesToolCallChunk:
    ...
    def to_response_payload(self) -> dict[str, JsonValue]:
...
def _event_type(event: dict[str, JsonValue], sse_event: str | None) -> str:
    value = event.get("type") or event.get("event") or sse_event
    return value if isinstance(value, str) else ""


def _event_key(event: dict[str, JsonValue]) -> str:
...
def _json_response_field(event: dict[str, JsonValue]) -> Mapping[str, JsonValue] | None:
...
def _terminal_error_message(provider_label: str, event_type: str, event: dict[str, JsonValue]) -> str | None:
...
def _merge_stream_item(
    event_type: str,
    event_mapping: dict[str, JsonValue],
    accumulator: ResponsesStreamAccumulator,
) -> None:
