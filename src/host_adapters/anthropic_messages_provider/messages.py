...
def build_request(
    ...
    *,
    ...
    overrides = options.parameter_overrides.get("anthropic_messages", "chat_completion")
...
def convert_response(response: object, *, options: RuntimeOptionsView) -> ProviderResponse:
    ...
    for block in response_model.content:
        if block.type == "text" and block.text:
            ...
            tool_use_id, tool_use_name = block.require_tool_use_identity()
            tool_calls.append(
                ProviderToolCall(
                    id=tool_use_id,
                    function=ProviderFunctionCall(name=tool_use_name, arguments=block.input),
