...
def _contains_dashscope_parameter(value: object, field_name: str) -> bool:
    ...
    parameters = json_mapping_or_none(mapping.get("parameters"))
    if parameters is not None and parameters.get(field_name) is not None:
        return True

    body = json_mapping_or_none(mapping.get("body"))
    if body is None:
        return False
    if body.get(field_name) is not None:
        return True
    body_parameters = json_mapping_or_none(body.get("parameters"))
    return body_parameters is not None and body_parameters.get(field_name) is not None


def _has_explicit_dashscope_max_tokens(context: TranslationContext) -> bool:
    """区分用户指定的原生旧字段与 Host 通用输出预算。"""

    sources: list[object] = [context.policy.default_params, context.policy.override_params]
    request = context.request
    if request is not None:
        if context.policy.accept_model_extra_params:
            sources.append(request.model_info.extra_params.fields)
        if context.policy.accept_request_extra_params:
            sources.append(request.extra_params.fields)
    return any(_contains_dashscope_parameter(source, "max_tokens") for source in sources)


def _can_auto_use_max_completion_tokens(context: TranslationContext) -> bool:
    target_path = "body.parameters.max_completion_tokens"
    blocked_paths = {
        path.strip() for path in (*context.policy.disabled_paths, *context.policy.rejected_paths) if path.strip()
    }
    return target_path not in blocked_paths
...
def normalize_dashscope_chat_body(body: dict, *, context: TranslationContext) -> None:
...
def _normalize_dashscope_token_limit(parameters: dict, *, context: TranslationContext) -> None:
    ...
    if legacy_value is None or explicit_legacy:
        return
    if not _can_auto_use_max_completion_tokens(context):
        return
...
def _normalize_dashscope_tool_choice(parameters: dict) -> None:
