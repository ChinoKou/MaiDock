...
def convert_tools(tool_options: list[ToolOptionSnapshot]) -> list[dict]:
    """把 Host 工具定义转换为 Chat Completions tools。"""
    tools: list[dict] = []
...
def extract_tool_calls(
    ...
    *,
    ...
    """从 Chat Completions 响应中提取 ProviderToolCall。"""
    ...
    for index, item in enumerate(tool_call_items, start=1):
        ...
        tool_calls.append(
            ProviderToolCall(
                id=resolve_tool_call_id(call_id, fallback_prefix=fallback_prefix, index=index),
