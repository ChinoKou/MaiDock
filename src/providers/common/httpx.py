...
async def post_json(
    ...
    *,
    ...
    for attempt in range(retries + 1):
        try:
            response = await client.post(path, json=json_body, headers=_headers(headers), params=_query_params(query))
        except httpx.TimeoutException as exc:
            if attempt < retries:
                continue
            raise HttpxProviderError(f"{provider_label} 上游接口请求超时: {exc}") from exc
        except httpx.HTTPError as exc:
            if attempt < retries:
                continue
            raise HttpxProviderError(f"{provider_label} 上游接口连接失败: {exc}") from exc
    ...
    raise HttpxProviderError(f"{provider_label} 上游接口调用失败")
